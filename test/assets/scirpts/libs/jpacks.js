...
var jpacks = (function (exportName) {
  /**
   * @file jpacks
   *
  ...
  function createSchema() {
    ...
    //从内存中解包出结构体数组
    Schema.unpackarray = function (schema, data, start, offset) {
      offset = offset || [0];
      start = start || 0;
      var length = data.byteLength - start
      var ret = [];
      while (offset[0] < length) {
        var item = Schema.unpack(schema, data, null, offset);
        ret.push(item)
      }
      return ret;
    };
  ...
  function create() {
    ...
    function encodeUTF8(str) {
      ...
      var ret = Uint8ArrayToString(arr);
      return ret;
    }
    /*</function>*/
    ...
    function decodeUTF8(str) {
      ...
      var iconv = require("iconv-lite")
      var ret = iconv.decode(arr, 'gb2312');
      return ret;
    }
    ...
    function bigintCreator(unsigned) {
      return new Schema({
        unpack: function (buffer, options, offsets) {
          ...
          // cc.log("max num = ", Number.MAX_SAFE_INTEGER);
          // if(cc.sys.os == cc.sys.OS_IOS)
          //   return Number(bigint.low);
          // return long.fromValue(bigint).toString();
          // return long.fromValue(bigint);
          // return Number(long.fromValue(bigint).toString());
          return (new long(bigint.low, bigint.high, bigint.unsigned)).toNumber()
          // return Number(jsbi.BigInt((bigint.high).toString() + (bigint.low).toString()));
        },
        pack: function _pack(value, options, buffer) {
